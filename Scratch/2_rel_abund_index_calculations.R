...
write.csv(RF_town_assess_sum,paste0("Scratch/Outputs/RF_perfomance.csv"))




#store each detection probability time series as a dataframe for each town polygon
ebirdRF_results_town_sf2<-list()
for (j in 1:length(name_list$sp_file)) {
  
  #splits towns and time series into separate dataframes
  ts_list<-ebirdRF_results_town[[j]]%>%
    dplyr::select(Year,city,week,date,eBird.DP.RF,eBird.DP.RF.SE)%>%
    group_split(city,.keep=F)
  #name the list of dataframes by town names
  names(ts_list)<-unique(ebirdRF_results_town[[j]][["city"]])
  ma_town_temp<-arrange(ma_towns,city)
  #add list of dataframes as a new attribute for the town layer
  ma_town_temp$DP_ts<-ts_list
  ebirdRF_results_town_sf2[[j]]<-ma_town_temp
  
}  

save(ebirdRF_results_town_sf2,ebirdRF_results_town,
     file="Scratch/Outputs/3_weekly_rel_abund_predictions.rda")
